...
do
  ...
  WIS=$(echo "$output" | grep "Nondiscounted WIS Expected average performance of the pruned FSM" | sed 's/[0-9]*.[^0-9]*//')
  OIS=$(echo "$output" | grep "Nondiscounted OIS Expected average performance of the pruned FSM" | sed 's/[0-9]*.[^0-9]*//')
